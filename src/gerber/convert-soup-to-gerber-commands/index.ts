...
const getBoardExtents = (
  board: Extract<AnyCircuitElement, { type: "pcb_board" }> | undefined,
) => {
...
const getCircleCutoutEdgeSide = (
  ...
  boardExtents: NonNullable<ReturnType<typeof getBoardExtents>>,
) => {
  const epsilon = 1e-9
  if (Math.abs(cutout.center.x - boardExtents.minX) < epsilon) {
    return "left"
  }
  if (Math.abs(cutout.center.x - boardExtents.maxX) < epsilon) {
    return "right"
  }
  if (Math.abs(cutout.center.y - boardExtents.minY) < epsilon) {
    return "bottom"
  }
  if (Math.abs(cutout.center.y - boardExtents.maxY) < epsilon) {
    return "top"
  }
}
